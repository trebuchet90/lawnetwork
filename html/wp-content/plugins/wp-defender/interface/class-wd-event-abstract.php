...
abstract class WD_Event_Abstract extends WD_Component {
	...
	public function build_log_data() {
		/**
		 * we will open a socket here, if none opened. need to load it here to apply the on-demand
		 */
		WD_Audit_API::open_socket();
		...
		if ( isset( $hook_data['callback'] ) && ! empty( $hook_data['callback'] ) ) {
			...
			$ret               = $reflection_method->invokeArgs( new WD_Audit_API(), array( $hook_name, $params ) );
			...
			if ( isset( $hook_data['context'] ) ) {
				if ( is_callable( $hook_data['context'] ) ) {
					...
					$context           = $reflection_method->invokeArgs( new WD_Audit_API(), array(
						$hook_name,
						$params
					) );
		...
		WD_Audit_API::queue_events_data( $post );
	}
	...
	private static function get_default_params() {
		return array(
			'wp_user'    => is_user_logged_in() ? WD_Utils::get_user_name( get_current_user_id() ) : null,
